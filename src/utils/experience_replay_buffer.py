...
class ExperienceReplayBuffer:
    ...
    def sample_batch(self, batch_size, device):
        ...
        state_sample = torch.tensor(self.state_buffer[indices]).float().to(device)
        action_sample = torch.tensor(self.action_buffer[indices]).long().to(device)
        reward_sample = torch.tensor(self.reward_buffer[indices]).float().to(device)
        next_state_sample = torch.tensor(self.next_state_buffer[indices]).float().to(device)
        done_sample = torch.tensor(self.done_buffer[indices]).float().to(device)
